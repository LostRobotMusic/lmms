...
#define setwavemodel( name )\
		...
		name->addItem( tr( "Sync" ), make_unique<PluginPixmapLoader>( "sin" ) );\
		name->addItem( tr( "Sync Half Interpolate" ), make_unique<PluginPixmapLoader>( "sin" ) );\
		name->addItem( tr( "Sync Interpolate" ), make_unique<PluginPixmapLoader>( "sin" ) );\
...
#define loadmodemodel( name )\
		...
		name.addItem( tr( "Autocorrelation method (static pitch detection)" ) );
/*		name.addItem( tr( "Choose waveform length" ) );\
		name.addItem( tr( "Similarity Test Method" ) );\*/

#define interpolatemodel( name )\
		name.clear();\
		name.addItem( tr( "Sample and Hold" ), make_unique<PluginPixmapLoader>( "number_1" ) );\
...
private:
	...
	bool keytrackingArr[8] = {true, true, true, true, true, true, true, true};
	float panArr[8] = {0};
	float tempoArr[8] = {0};
